...
-- The original migration 20260129000004 may not have been applied.
...
CREATE TABLE IF NOT EXISTS sync_configurations (
  ...
  sync_frequency TEXT DEFAULT 'daily' CHECK (sync_frequency IN ('hourly', 'daily', 'weekly', 'manual', 'on_login')),
...
CREATE INDEX IF NOT EXISTS idx_sync_configs_event ON sync_configurations(event_id);
CREATE INDEX IF NOT EXISTS idx_sync_configs_next_sync ON sync_configurations(next_sync_at) WHERE is_active = true;

-- Enable RLS
ALTER TABLE sync_configurations ENABLE ROW LEVEL SECURITY;

-- RLS Policies (use DO block to avoid errors if already exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'sync_configurations' AND policyname = 'Users can view org sync configs'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'sync_configurations' AND policyname = 'MD can manage sync configs'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_sync_configs_updated_at'
  ) THEN
