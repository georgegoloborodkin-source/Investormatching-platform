...
export default function CIS() {
  ...
  // Log Connection dialog form state (autofilled from chat/suggestion when opening)
  const [logConnSource, setLogConnSource] = useState("");
  const [logConnTarget, setLogConnTarget] = useState("");
  const [logConnType, setLogConnType] = useState<ConnectionType>("BD");
  const [logConnStatus, setLogConnStatus] = useState<ConnectionStatus>("To Connect");
  const [logConnNotes, setLogConnNotes] = useState("");
  const [logConnSaving, setLogConnSaving] = useState(false);
  ...
  // When Log Connection dialog opens, autofill form from pending context + chat thread
  useEffect(() => {
    if (!logDecisionDialogOpen) return;
    setLogConnSource(pendingDecisionContext?.source_company_name ?? "");
    setLogConnTarget(pendingDecisionContext?.target_company_name ?? "");
    setLogConnType((pendingDecisionContext?.connection_type as ConnectionType) ?? "BD");
    setLogConnStatus("To Connect");
    const threadContext = scopedMessages.slice(-4).map((m) =>
      `${m.author === "user" ? "User" : "Assistant"}: ${m.text.slice(0, 300)}${m.text.length > 300 ? "..." : ""}`
    ).join("\n\n");
    const reasoning = pendingDecisionContext?.aiReasoning ?? "";
    setLogConnNotes([threadContext, reasoning].filter(Boolean).join("\n\n---\n\n"));
  }, [logDecisionDialogOpen, pendingDecisionContext?.source_company_name, pendingDecisionContext?.target_company_name, pendingDecisionContext?.connection_type, pendingDecisionContext?.aiReasoning, scopedMessages]);
  ...
  // Handler to open Log Connection dialog from chat (with optional connection autofill)
  const handleLogDecisionFromChat = useCallback((
    aiReasoning: string,
    sourceDocIds?: string[],
    connectionAutofill?: { source_company_name?: string; target_company_name?: string; connection_type?: ConnectionType }
  ) => {
  ...
  // Open Log Connection dialog from AI suggestion (one-click autofill)
  const handleLogConnectionFromSuggestion = useCallback((suggestion: {
    ...
    const typeMap: Record<string, ConnectionType> = {
      BD: "BD", INV: "INV", Knowledge: "Knowledge", Partnership: "Partnership", Portfolio: "Portfolio",
    };
    setPendingDecisionContext({
      aiReasoning: suggestion.reasoning,
      source_company_name: suggestion.source_company,
      target_company_name: suggestion.target_company,
      connection_type: typeMap[suggestion.connection_type] ?? "BD",
    });
    setLogDecisionDialogOpen(true);
  }, []);
  ...
  // Log Connection dialog submit
  const submitLogConnection = useCallback(async () => {
    if (!logConnSource.trim() || !logConnTarget.trim()) {
      toast({ title: "Missing fields", description: "Source and target company are required.", variant: "destructive" });
      return;
    }
    setLogConnSaving(true);
    try {
      await handleCreateConnection({
        source_company_name: logConnSource.trim(),
        target_company_name: logConnTarget.trim(),
        connection_type: logConnType,
        connection_status: logConnStatus,
        ai_reasoning: logConnNotes.trim() || null,
        notes: logConnNotes.trim() || null,
      });
    } finally {
      setLogConnSaving(false);
    }
  }, [logConnSource, logConnTarget, logConnType, logConnStatus, logConnNotes, handleCreateConnection, toast]);
  ...
  const renderAssistantContent = useCallback((text: string) => {
    ...
    function renderInline(raw: string, keyPrefix: string = ""): React.ReactNode[] {
      ...
      while ((match = inlineRegex.exec(processedText)) !== null) {
        ...
        if (match.index > lastIndex) {
          ...
          const group = match[2] ?? match[3] ?? match[4] ?? match[5] ?? match[6] ?? fullMatch;
          parts.push(group);
          lastIndex = match.index + fullMatch.length;
        }
      }
      const tail = processedText.slice(lastIndex);
      if (linkPlaceholders && Object.keys(linkPlaceholders).length > 0) {
        const tailParts = tail.split(/(__LINK_\d+__)/g);
    ...
    return (
      <>
        {renderInline(text).map((node, idx) => (
  ...
  const CONNECTION_TYPES_DIALOG: ConnectionType[] = ["BD", "INV", "Knowledge", "Partnership", "Portfolio"];
  const CONNECTION_STATUSES_DIALOG: ConnectionStatus[] = ["To Connect", "In Progress", "Connected", "Rejected", "Completed"];

  return (
    <>
      {/* Log Connection dialog (one-click from chat or AI suggestion) */}
      <Dialog
        open={logDecisionDialogOpen}
        onOpenChange={(open) => {
          if (!open) {
            setLogDecisionDialogOpen(false);
            setPendingDecisionContext(null);
        ...
        <DialogContent className="bg-[#050505] border border-border text-white max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-foreground font-mono font-bold uppercase tracking-tight flex items-center gap-2">
              ...
              Log Company Connection
            </DialogTitle>
            <DialogDescription className="text-muted-foreground font-mono text-xs">
              Source and target companies, connection type, and optional context from the chat thread are prefilled when opened from chat or a suggestion.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label className="text-foreground font-mono font-bold text-xs uppercase tracking-wider">Source company</Label>
              <Input
                className="mt-1 border border-border bg-card text-foreground"
                value={logConnSource}
                onChange={(e) => setLogConnSource(e.target.value)}
                placeholder="e.g. Acme Corp"
              />
            </div>
            <div>
              <Label className="text-foreground font-mono font-bold text-xs uppercase tracking-wider">Target company</Label>
              <Input
                className="mt-1 border border-border bg-card text-foreground"
                value={logConnTarget}
                onChange={(e) => setLogConnTarget(e.target.value)}
                placeholder="e.g. Beta Inc"
              />
            </div>
            <div>
              <Label className="text-foreground font-mono font-bold text-xs uppercase tracking-wider">Connection type</Label>
              <Select value={logConnType} onValueChange={(v) => setLogConnType(v as ConnectionType)}>
                <SelectTrigger className="mt-1 border border-border bg-card text-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#050505] border border-border">
                  {CONNECTION_TYPES_DIALOG.map((t) => (
                    <SelectItem key={t} value={t} className="text-foreground font-mono">{t}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label className="text-foreground font-mono font-bold text-xs uppercase tracking-wider">Status</Label>
              <Select value={logConnStatus} onValueChange={(v) => setLogConnStatus(v as ConnectionStatus)}>
                <SelectTrigger className="mt-1 border border-border bg-card text-foreground">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#050505] border border-border">
                  {CONNECTION_STATUSES_DIALOG.map((s) => (
                    <SelectItem key={s} value={s} className="text-foreground font-mono">{s}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            <div>
              <Label className="text-foreground font-mono font-bold text-xs uppercase tracking-wider">Additional context</Label>
              <Textarea
                className="mt-1 min-h-[120px] border border-border bg-card text-foreground font-mono text-sm"
                value={logConnNotes}
                onChange={(e) => setLogConnNotes(e.target.value)}
                placeholder="Thread summary and AI reasoning (autofilled when opened from chat)"
              />
            </div>
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <Button
              variant="outline"
              className="border border-border bg-card text-foreground hover:bg-muted/50"
              onClick={() => {
                setLogDecisionDialogOpen(false);
                setPendingDecisionContext(null);
              }}
            ...
            <Button
              ...
              disabled={logConnSaving}
              onClick={() => submitLogConnection()}
            >
              {logConnSaving ? "Saving..." : "Save connection"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
